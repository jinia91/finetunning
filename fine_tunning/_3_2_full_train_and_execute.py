...
# Full fine-tuning 설정 (LoRA 관련 설정 제거)
...
training_params = TrainingArguments(
    num_train_epochs=1, # 데이터 세트 반복 횟수
    max_steps=1, # 총 합습 스텝수
    per_device_train_batch_size=1, # GPU당 배치 사이즈
    gradient_accumulation_steps=1, # 배치 사이즈를 늘리기 위한 그래디언트 누적 스텝수ㅏ, 실제 배치 크기는 batch_size * gradient_accumulation_steps
    optim=optim_name, # 최적화 알고리즘
    warmup_ratio=0.03,
    learning_rate=2e-4, # 옵티마이저 기본 학습률
    lr_scheduler_type="cosine", # 학습률 변화 방식, cosine은 코사인 형태로 학습률이 감소함
    weight_decay=0.0, # 가중치 감소율, 일반적으로 0.01~0.1 사이 과적합 방지용
    bf16=False,
    fp16=False,
...
# SFTTrainer 설정 (Full fine-tuning)
trainer = SFTTrainer(
    model=model,
